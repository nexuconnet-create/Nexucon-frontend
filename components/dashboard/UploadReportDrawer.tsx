...
import { X, FileText, UploadCloud, Folder, CheckCircle, HardDrive, FileBarChart, BookOpen, ShieldCheck } from 'lucide-react';
import { createDocument, getDocumentFolders, DocumentFolder } from '@/services/documents';
import { getProjects, Project } from '@/services/projects';

interface UploadReportDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
...
const DEFAULT_REPORT_FOLDERS = [
  { value: '01_Geotechnical_Studies', label: '01_Geotechnical Studies & Soil Investigations' },
  { value: '02_Environmental_Assessments', label: '02_Environmental Impact Assessments (EIA)' },
  { value: '03_Structural_Calculations', label: '03_Structural Calculations & Load Modeling' },
  { value: '04_Hydrological_Drainage', label: '04_Hydrological & Flood Risk Studies' },
  { value: '05_Traffic_Transport_Studies', label: '05_Traffic Impact & Transportation Analysis' },
  { value: '06_Quality_Assurance_Reports', label: '06_Quality Assurance & Material Test Records' }
];

export default function UploadReportDrawer({
  isOpen,
  onClose,
  onSuccess,
  defaultProjectId,
  ...
  useEffect(() => {
    ...
    getProjects()
      ...
      .catch(err => console.error("Failed to load projects for report upload", err));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedFile) {
        ...
        formData.append('uploader_name', authorName || 'Lead Geotechnical Consultant');
        ...
        await createDocument({
          ...
          title: title || 'Comprehensive Technical Engineering Assessment',
          folder,
          document_type: 'TECHNICAL_REPORT' as any,
          discipline: discipline as any,
          uploader_name: authorName || 'Lead Consultant',
          file_format: fileFormat,
          file_size: fileSize,
          pages_count: pagesCount,
          current_version: versionLabel,
          expiry_date: expiryDate || undefined,
          file_url: `https://ba64cd9c51c2da4db93a1886397fd7b3.r2.cloudflarestorage.com/nexucondocument/projects/reports/${title.replace(/ /g, '_')}.pdf`
        });
      }

      window.dispatchEvent(new CustomEvent('show-toast', { 
        detail: { message: 'Technical Report successfully uploaded & registered in Cloudflare R2!', type: 'success' } 
      }));
      onClose();
      if (onSuccess) onSuccess();
    } catch (err: any) {
      const respData = err.response?.data;
      let msg = 'Failed to upload technical report';
      ...
      <div className="fixed right-0 top-0 bottom-0 w-full max-w-[580px] bg-white p-8 shadow-2xl flex flex-col z-[101] animate-in slide-in-from-right-8 duration-300 border-l border-slate-200">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 className="text-lg font-bold text-[#022C4F]">
                  Upload Technical Engineering Report
                </h2>
                <p className="text-xs text-slate-500">Statutory engineering study ingestion into Cloudflare R2 (Bucket: nexucondocument)</p>
              </div>
            </div>
          </div>
          <button 
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto py-6 space-y-5 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
                Report Folder <span className="text-red-500">*</span>
              </label>
              <select
                value={folder}
              ...
              >
                {DEFAULT_REPORT_FOLDERS.map((f) => (
                  <option key={f.value} value={f.value}>{f.value}</option>
                ))}
                {projectFolders
                  .filter(pf => !DEFAULT_REPORT_FOLDERS.some(df => df.value === pf.name))
                  .map((pf) => (
            ...
            <div>
              ...
              >
                ...
                <option value="Planning">Planning & Urban Design</option>
              </select>
            </div>
          </div>
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
              Report Title / Study Description <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              required
              ...
              placeholder="e.g. Geotechnical Soil Investigation & Deep Piling Feasibility Report"
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
              Author / Lead Consultant / Firm
            </label>
            <input
              type="text"
              value={authorName}
              onChange={(e) => setAuthorName(e.target.value)}
              placeholder="e.g. Trevi Foundation Geotechnics Ltd"
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium"
            />
          </div>

          {/* Version Label & Pages Count */}
          ...
          <div>
            ...
            >
              ...
              {selectedFile ? (
                ...
                <div>
                  <p className="text-sm font-bold text-slate-700">Click to browse or drag and drop report file</p>
                  <p className="text-xs text-slate-400 mt-1">Accepts PDF engineering reports, Word documents, and spreadsheets</p>
                </div>
              )}
            </div>
          </div>
