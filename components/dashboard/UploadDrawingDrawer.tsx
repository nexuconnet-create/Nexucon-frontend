...
import { X, FileText, UploadCloud, Folder, CheckCircle, HardDrive, Layers, Compass, ShieldCheck, Tag } from 'lucide-react';
import { createDocument, getDocumentFolders, DocumentFolder } from '@/services/documents';
import { getProjects, Project } from '@/services/projects';

interface UploadDrawingDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
...
const DEFAULT_DRAWING_FOLDERS = [
  { value: '01_Architectural', label: '01_Architectural (Floor Plans, Elevations, Sections)' },
  { value: '02_Structural', label: '02_Structural (Piling, Foundations, Beams & Columns)' },
  { value: '03_MEP_Systems', label: '03_MEP_Systems (HVAC, Electrical, Plumbing, Fire)' },
  { value: '04_Planning', label: '04_Planning (Urban Design, Zoning, Master Site Layout)' },
  { value: '05_Civil_Infrastructure', label: '05_Civil_Infrastructure (Roads, Drainage, Grading)' }
];

export default function UploadDrawingDrawer({
  isOpen,
  onClose,
  onSuccess,
  defaultProjectId,
  ...
  const [discipline, setDiscipline] = useState(defaultDiscipline || 'Architecture');
  ...
  useEffect(() => {
    ...
    getProjects()
      ...
      .catch(err => console.error("Failed to load projects for drawing upload", err));
    ...
    try {
      if (selectedFile) {
        ...
        await createDocument({
          ...
          title: title || '2D Architectural Plan & Elevation',
          folder,
          document_type: 'DRAWING' as any,
          ...
          file_url: `https://ba64cd9c51c2da4db93a1886397fd7b3.r2.cloudflarestorage.com/nexucondocument/projects/drawings/${title.replace(/ /g, '_')}.pdf`
        });
      }

      window.dispatchEvent(new CustomEvent('show-toast', { 
        detail: { message: '2D Drawing successfully uploaded & stored in Cloudflare R2!', type: 'success' } 
      }));
      onClose();
      if (onSuccess) onSuccess();
    } catch (err: any) {
      const respData = err.response?.data;
      let msg = 'Failed to upload 2D drawing';
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 className="text-lg font-bold text-[#022C4F]">
                  Upload 2D Drawing & Blueprint
                </h2>
                <p className="text-xs text-slate-500">Official statutory drawing submission to Cloudflare R2 bucket (nexucondocument)</p>
              </div>
            </div>
          </div>
          <button 
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto py-6 space-y-5 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              >
                {DEFAULT_DRAWING_FOLDERS.map((f) => (
                  <option key={f.value} value={f.value}>{f.value}</option>
                ))}
                {projectFolders
                  .filter(pf => !DEFAULT_DRAWING_FOLDERS.some(df => df.value === pf.name))
                  .map((pf) => (
            ...
            <div>
              ...
              >
                ...
                <option value="MEP">MEP (Mech / Elec / Plumbing)</option>
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
              Drawing Title / Sheet Name <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              required
              ...
              placeholder="e.g. AR-DWG-001: General Arrangement & Floor Plans"
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
              Blueprint / Drawing File (.PDF, .DWG, .DXF, .SVG)
            </label>
            <input 
              type="file" 
              ref={fileInputRef}
              onChange={handleFileChange}
              accept=".pdf,.dwg,.dxf,.png,.jpg,.jpeg,.svg,.tiff"
              className="hidden" 
            />
            ...
            >
              ...
              {selectedFile ? (
                ...
                <div>
                  <p className="text-sm font-bold text-slate-700">Click to browse or drag and drop drawing</p>
                  <p className="text-xs text-slate-400 mt-1">Accepts statutory PDF blueprints, AutoCAD DWG/DXF, and high-res schematics</p>
                </div>
              )}
            </div>
          </div>
