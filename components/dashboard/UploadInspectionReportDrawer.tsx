...
import { X, ClipboardList, UploadCloud, Folder, CheckCircle, HardDrive, ShieldAlert, CheckCircle2 } from 'lucide-react';
import { createDocument, getDocumentFolders, DocumentFolder } from '@/services/documents';
import { getProjects, Project } from '@/services/projects';

interface UploadInspectionReportDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
...
const DEFAULT_INSPECTION_FOLDERS = [
  { value: '06_Site_Inspections', label: '06_Site_Inspections (General QA/QC Field Logs)' },
  { value: '01_Foundation_Inspections', label: '01_Foundation & Piling Integrity Verification' },
  { value: '02_Structural_Framing', label: '02_Structural Framing & Rebar Placement Inspections' },
  { value: '03_MEP_Commissioning', label: '03_MEP Fire & Safety Commissioning' },
  { value: '04_HSE_Audits', label: '04_HSE Environmental & Safety Audits' }
];

export default function UploadInspectionReportDrawer({
  isOpen,
  onClose,
  onSuccess,
  defaultProjectId,
  defaultFolder,
  defaultDiscipline
}: UploadInspectionReportDrawerProps) {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [projects, setProjects] = useState<Project[]>([]);
  const [projectFolders, setProjectFolders] = useState<DocumentFolder[]>([]);
  const [selectedProjectId, setSelectedProjectId] = useState(defaultProjectId || '');
  const [title, setTitle] = useState('');
  const [folder, setFolder] = useState(defaultFolder || '06_Site_Inspections');
  const [discipline, setDiscipline] = useState(defaultDiscipline || 'Structural');
  const [inspectorName, setInspectorName] = useState('Engr. Tunde Adeleke (Lead Field Inspector)');
  const [inspectionOutcome, setInspectionOutcome] = useState('APPROVED');
  const [versionLabel, setVersionLabel] = useState('v1.0');
  const [fileFormat, setFileFormat] = useState('PDF');
  const [fileSize, setFileSize] = useState('11.8 MB');
  ...
  useEffect(() => {
    ...
    getProjects()
      ...
      .catch(err => console.error("Failed to load projects for inspection upload", err));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedFile) {
        ...
        formData.append('status', inspectionOutcome);
        formData.append('uploader_name', inspectorName || 'Lead Inspector');
        ...
        await createDocument({
          ...
          title: title || 'Official Site QA/QC Inspection Record',
          folder,
          document_type: 'INSPECTION_REPORT' as any,
          discipline: discipline as any,
          status: inspectionOutcome as any,
          uploader_name: inspectorName || 'Lead Inspector',
          file_format: fileFormat,
          file_size: fileSize,
          pages_count: pagesCount,
          current_version: versionLabel,
          file_url: `https://ba64cd9c51c2da4db93a1886397fd7b3.r2.cloudflarestorage.com/nexucondocument/projects/inspections/${title.replace(/ /g, '_')}.pdf`
        });
      }

      window.dispatchEvent(new CustomEvent('show-toast', { 
        detail: { message: 'Inspection Report successfully uploaded & registered in Cloudflare R2!', type: 'success' } 
      }));
      onClose();
      if (onSuccess) onSuccess();
    } catch (err: any) {
      const respData = err.response?.data;
      let msg = 'Failed to upload inspection report';
    ...
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 w-full max-w-[580px] bg-white p-8 shadow-2xl flex flex-col z-[101] animate-in slide-in-from-right-8 duration-300 border-l border-slate-200">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 className="text-lg font-bold text-[#022C4F]">
                  Upload Inspection Report & QA/QC Audit
                </h2>
                <p className="text-xs text-slate-500">Site verification, QA/QC audits & engineering inspections (Bucket: nexucondocument)</p>
              </div>
            </div>
          </div>
          <button 
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto py-6 space-y-5 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
              Inspection Report Title <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              required
              ...
              placeholder="e.g. INSP-2026-QA04: 18th Floor Slab Post-Pour Inspection & Core Test"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
                Inspection Folder <span className="text-red-500">*</span>
              </label>
              <select
                value={folder}
              ...
              >
                {DEFAULT_INSPECTION_FOLDERS.map((f) => (
                  <option key={f.value} value={f.value}>{f.value}</option>
                ))}
                {projectFolders
                  .filter(pf => !DEFAULT_INSPECTION_FOLDERS.some(df => df.value === pf.name))
                  .map((pf) => (
            ...
            <div>
              ...
              >
                ...
                <option value="MEP">MEP & Fire Protection</option>
                <option value="Civil">Civil Infrastructure</option>
                <option value="Environmental">HSE & Environmental</option>
                <option value="Architecture">Architectural Finishes</option>
              </select>
            </div>
          </div>

          {/* Inspector Name & Audit Outcome */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium cursor-pointer"
              >
                <option value="APPROVED">Passed (Compliant with Building Code)</option>
                <option value="PENDING_REVIEW">Under Review / Minor Punch List</option>
                <option value="CHANGES_REQUESTED">Action Required / NCR Logged</option>
                <option value="REJECTED">Failed / Non-Conformance Notice</option>
              </select>
            </div>
          </div>
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">
              Inspection Report Document (.PDF, .DOCX, .XLSX)
            </label>
            <input 
              type="file" 
              ref={fileInputRef}
            ...
            >
              ...
              {selectedFile ? (
                ...
                <div>
                  <p className="text-sm font-bold text-slate-700">Click to browse or drag and drop inspection report</p>
                  <p className="text-xs text-slate-400 mt-1">Accepts signed QA/QC audit sheets, field test logs, and photographic inspection records</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500">
                Stored under <code className="font-mono text-blue-700">projects/{selectedProjectObj?.reference_number || 'PRJ'}/{folder}</code>
              </p>
            </div>
          </div>
          ...
          <div className="pt-4 flex items-center justify-end gap-3 border-t border-slate-100">
            ...
            <button
              ...
              className="px-6 py-2.5 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-lg shadow-blue-600/20 transition-all flex items-center gap-2 cursor-pointer disabled:opacity-50"
            >
              {isSubmitting ? (
                <>
