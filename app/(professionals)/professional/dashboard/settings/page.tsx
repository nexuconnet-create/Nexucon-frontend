...
export default function SettingsPage() {
  const activeTab: string = "Project Settings";
  ...
  const [offlineSettings, setOfflineSettings] = useState<Record<string, boolean>>({
    "Enable Offline Mode": true,
    "Cache Critical Documents": true,
    "Cache Active BIM Models": true,
    "Sync Pending Changes on Reconnect": true,
  });
  ...
  const toggleBimSetting = (setting: string) => {
    setBimSettings(prev => ({ ...prev, [setting]: !prev[setting] }));
  };

  const toggleOfflineSetting = (setting: string) => {
    setOfflineSettings(prev => ({ ...prev, [setting]: !prev[setting] }));
  };
      ...
      {activeTab === "Project Settings" && (
        <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-16 mt-6">
          ...
          <div className="flex flex-col gap-16 lg:pl-12">
            ...
            <div className="flex flex-col gap-8">
              <h2 className="text-[20px] font-extrabold text-[#022C4F]">Offline Access Management</h2>
              <div className="flex flex-col gap-5">
                {Object.keys(offlineSettings).map((setting) => (
                  <label key={setting} className="flex items-center gap-4 cursor-pointer group">
                    <div 
                      className={`w-6 h-6 rounded flex items-center justify-center shrink-0 transition-colors ${
                        offlineSettings[setting] ? "bg-[#0F181F] border-[#0F181F]" : "border-gray-400 border group-hover:border-[#0F181F] bg-white"
                      }`}
                      onClick={() => toggleOfflineSetting(setting)}
                    >
                      {offlineSettings[setting] && <svg className="w-3.5 h-3.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>}
                    </div>
                    <span className="text-[12px] text-[#0F181F] font-medium">{setting}</span>
                  </label>
                ))}
