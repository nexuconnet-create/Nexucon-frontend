...
export default function GovernmentForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!cleanEmail) {
      setError("Please enter your official agency email address.");
      return;
    }
    ...
    try {
      ...
      console.error("Government password reset error:", err);
      const serverMsg = err.response?.data?.message || "Failed to dispatch recovery instructions. Please try again.";
      setError(serverMsg);
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="hidden lg:flex relative z-10 w-1/2 h-full flex-col justify-between p-10 min-h-[calc(100vh)] text-white">
        ...
        <div className="max-w-xl pb-16 pt-0">
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-500/20 border border-blue-300/30 text-xs font-bold text-blue-200 uppercase tracking-wider mb-4">
            <ShieldCheck size={14} />
            Official Government Regulatory Portal
          </div>

          <h1 className="text-[38px] font-extrabold text-white mb-4 leading-tight">
            Agency Access &amp; Password Recovery
          </h1>

          <p className="text-white/80 text-base font-medium leading-relaxed max-w-lg mb-8">
            Restore access to statutory building oversight, stage inspection workflows, plan reviews, and jurisdiction compliance records with encrypted token authentication.
          </p>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="p-3.5 rounded-xl bg-white/10 backdrop-blur-md border border-white/10">
              ...
              <div className="text-xs font-bold text-white">Encrypted Reset</div>
              <div className="text-[11px] text-white/60 mt-0.5">Automated token dispatch</div>
            </div>
            <div className="p-3.5 rounded-xl bg-white/10 backdrop-blur-md border border-white/10">
              <Lock className="text-emerald-300 mb-2" size={20} />
              <div className="text-xs font-bold text-white">Zero Trust</div>
              <div className="text-[11px] text-white/60 mt-0.5">Role-based security</div>
            </div>
            <div className="p-3.5 rounded-xl bg-white/10 backdrop-blur-md border border-white/10">
              <ShieldCheck className="text-amber-300 mb-2" size={20} />
              <div className="text-xs font-bold text-white">Statutory Audit</div>
              <div className="text-[11px] text-white/60 mt-0.5">Tamper-proof log trail</div>
            </div>
          </div>
        </div>
            ...
            <>
              ...
              <div className="text-left mb-8">
                ...
                <p className="text-xs sm:text-sm font-medium text-gray-500 leading-relaxed">
                  Enter your official government or regulatory email address. We will dispatch secure password recovery instructions and a 6-digit verification code.
                </p>
                {error && (
            ...
            <div className="flex flex-col justify-between h-full py-4">
              <div className="text-left">
                ...
                <p className="text-xs sm:text-sm font-medium text-gray-600 leading-relaxed mb-6">
                  If <strong className="text-[#022C4F]">{email}</strong> is registered with our regulatory portal, you will receive an email shortly with your secure reset button and 6-digit verification code.
                </p>
                ...
                <div className="p-4 rounded-2xl bg-blue-50/70 border border-blue-100 text-xs text-blue-900 space-y-2">
                  ...
                  <ul className="list-disc pl-5 space-y-1 text-blue-800/90 font-medium">
                    <li>Check your official agency inbox (and junk/quarantine filter).</li>
                    <li>Click the link in the email or enter your 6-digit code on the reset page.</li>
                    <li>Recovery link and code remain active for 60 minutes.</li>
                  </ul>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3 mt-8">
                <Link
                  href={`/government/reset-password?email=${encodeURIComponent(email)}`}
